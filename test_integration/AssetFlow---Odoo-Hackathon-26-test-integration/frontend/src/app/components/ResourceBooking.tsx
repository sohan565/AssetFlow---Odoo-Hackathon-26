    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-4 text-xs mt-2 pl-0.5">
                 ...
                 {manager && b.status === "Pending" && (
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      ...
                      className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 text-xs text-emerald-200 hover:bg-emerald-400/15 transition-colors font-semibold"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => setBookingStatus(b.id, "Rejected")}
                      className="rounded-lg border border-rose-400/30 bg-rose-400/10 px-3 py-1.5 text-xs text-rose-200 hover:bg-rose-400/15 transition-colors font-semibold"
                    >
                      Reject
                    </button>
                  </div>
                )}
                {!manager && b.requestedBy === currentUser.id && (b.status === "Pending" || b.status === "Approved") && (
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      onClick={() => setBookingStatus(b.id, "Rejected")}
                      className="rounded-lg border border-rose-400/30 bg-rose-400/10 px-3 py-1.5 text-xs text-rose-200 hover:bg-rose-400/15 transition-colors font-semibold"
